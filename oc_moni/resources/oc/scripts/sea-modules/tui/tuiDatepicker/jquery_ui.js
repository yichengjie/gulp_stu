...
(function(b, f) {
	...
	b.fn
			.extend({
				...
				scrollParent : function() {
					var g;
					if ((b.ui.ie && (/(static|relative)/).test(this
							.css("position")))
						...
						g = this.parents().filter(
								function() {
									return (/(auto|scroll)/).test(b.css(this,
											"overflow")
											+ b.css(this, "overflow-y")
											+ b.css(this, "overflow-x"))
								}).eq(0)
					}
					return (/fixed/).test(this.css("position")) || !g.length ? b(document)
							: g
				},
				zIndex : function(j) {
					if (j !== f) {
	...
	function d(i, g) {
		...
		return (/input|select|textarea|button|object/.test(l) ? !i.disabled
				: "a" === l ? i.href || g : g)
				&& c(i)
	}
	function c(g) {
		return b.expr.filters.visible(g)
				&& !b(g).parents().andSelf().filter(function() {
	...
	b(function() {
		var g = document.body, h = g.appendChild(h = document
				.createElement("div"));
		h.offsetHeight;
		b.extend(h.style, {
			minHeight : "100px",
	...
	if (!b("<a>").outerWidth(1).jquery) {
		b.each([ "Width", "Height" ],
				function(j, g) {
					var h = g === "Width" ? [ "Left", "Right" ] : [ "Top",
							"Bottom" ], k = g.toLowerCase(), m = {
					...
					function l(o, n, i, p) {
						b.each(h, function() {
							...
							if (i) {
								n -= parseFloat(b.css(o, "border" + this
										+ "Width")) || 0
							}
							if (p) {
			...
			.extend(
					b.ui,
					{
						plugin : {
							...
							call : function(g, j, h) {
								...
								if (!l
										...
										|| g.element[0].parentNode.nodeType === 11) {
									return
								}
						...
						isOver : function(l, h, k, j, g, i) {
							return b.ui.isOverAxis(l, k, g)
									&& b.ui.isOverAxis(h, j, i)
						}
					})
})(jQuery);
(function($, undefined) {
	...
	LunarCalendar.prototype.setDate = function(date) {
		...
		this.SM = date.getMonth();
		this.SD = date.getDate();
		this.Lunar();
		return this
	};
	LunarCalendar.prototype.lunarInfo = [ 19416, 19168, 42352, 21717, 53856,
			...
			27808, 46416, 86869, 19872, 42416, 83315, 21168, 43432, 59728,
			27296, 44710, 43856, 19296, 43748, 42352, 21088, 62051, 55632,
			23383, 22176, 38608, 19925, 19152, 42192, 54484, 53840, 54616,
			46400, 46752, 103846, 38320, 18864, 43380, 42160, 45690, 27216,
	...
	LunarCalendar.prototype.Lunar = function() {
		var dateObj = this.dateObj, i, leap = 0, temp = 0, lunarObj = {}, baseDate = new Date(
				1900, 0, 31), offset = (dateObj - baseDate) / 86400000;
	...
	LunarCalendar.prototype.getYearToString = function() {
		var str1 = [ "甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸" ], str2 = [
				"子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥" ], y = (this.lunarDate.year) >>> 0, result;
		result = str1[(y - 4) % 10] + str2[(y - 4) % 12] + "年";
		return result
	};
	LunarCalendar.prototype.getMouthToString = function() {
		var str = [ " ", "正", "二", "三", "四", "五", "六", "七", "八", "九", "十",
				"十一", "腊" ], result, m = (this.lunarDate.month) >>> 0;
		if (m > 12 || m < 1) {
			return ""
		}
		result = str[m] + "月";
	...
	LunarCalendar.prototype.getDateToString = function() {
		var str1 = [ "初", "十", "廿", "三", "　" ], str2 = [ "十", "一", "二", "三",
				"四", "五", "六", "七", "八", "九" ], d = this.lunarDate.day, result, bits, ten;
		if (d > 39 || d < 0) {
			return ""
		}
		bits = (d % 10) >>> 0;
	...
	LunarCalendar.prototype.test = function() {
		var result = this.getYearToString() + this.getMouthToString()
			...
			.extend(
					Datepicker.prototype,
					{
						...
						_attachDatepicker : function(target, settings) {
							...
							settings
									&& settings.festivalList
									&& (settings.festivalList = $.extend({},
											this._defaults.festivalList,
											settings.festivalList));
						...
						_getDateFormatString : function(date) {
							var yyyy = date.getFullYear(), mm = date.getMonth() + 1, dd = date
									.getDate();
						...
						_generateHTML : function(inst) {
							...
							var hideIfNoPrevNext = this._get(inst,
									"hideIfNoPrevNext");
							var clearBtn = this._get(inst, "clearBtn");
							...
							var prev = (this._canAdjustMonth(inst, -1,
									drawYear, drawMonth) ? '<a class="ui-datepicker-prev ui-corner-all" data-handler="prev" data-event="click" title="'
									+ prevText
							...
							var next = (this._canAdjustMonth(inst, +1,
									drawYear, drawMonth) ? '<a class="ui-datepicker-next ui-corner-all" data-handler="next" data-event="click" title="'
									+ nextText
									+ '"><span class="ui-icon ui-icon-circle-triangle-'
									+ (isRTL ? "w" : "e")
									+ '">'
							...
							var buttonPanel = (showButtonPanel) ? '<div class="ui-datepicker-buttonpane ui-widget-content">'
									+ (isRTL ? controls : "")
									+ (this._isInRange(inst, gotoDate) ? '<button type="button" class="ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all" data-event="click" data-handler="'
											+ (clearBtn ? "clear" : "today")
											+ '">'
											+ (clearBtn ? this._get(inst,
							...
							var html = "";
							var isShowLunarCal = this._get(inst,
									"isShowLunarCal");
							var lunarCal = new LunarCalendar();
							var isShowFestival = this._get(inst,
									"isShowFestival");
							var festivalList = this._get(inst, "festivalList");
									...
									for ( var dRow = 0; dRow < numRows; dRow++) {
										...
										for ( var dow = 0; dow < 7; dow++) {
											...
											var formatString = this
													._getDateFormatString(printDate);
											var festivalString = (isShowFestival ? festivalList[formatString]
													: null);
											var festivalMark = null;
											if (festivalString) {
												festivalMark = festivalString
														.charAt(festivalString.length - 1);
												if (festivalMark == "*") {
													festivalString = festivalString
															.slice(
																	0,
																	festivalString.length - 1)
												} else {
													festivalMark = null
												}
											}
											...
											tbody += '<td class="'
													...
													+ (otherMonth
															...
															: (unselectable ? '<span class="ui-state-default">'
																	+ printDate
																			.getDate()
																	+ (isShowLunarCal ? "<br /><span class='tui_lunar_date'>"
																			+ (lunarCal.lunarDate.day == 1 ? lunarCal
																					.getMouthToString()
																					: lunarCal
																							.getDateToString())
																			+ "</span>"
																	...
																	: '<a class="ui-state-default'
																			...
																			+ printDate
																					.getDate()
																			+ '" title= "'
																			+ (isShowLunarCal ? (lunarCal
																					.getYearToString() + lunarCal
																					.getMouthToString())
																					: "")
																			+ (festivalString ? " "
																					+ festivalString
